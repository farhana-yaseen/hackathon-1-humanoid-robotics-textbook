...
class RAGService:
    ...
    def generate_streaming_response(self, session_id: str, question: str, context: str):
        """
        Generate a streaming response for the user's question.
        ...
        """
        try:
            logger.debug(f"Generating streaming response for session: {session_id}")
            answer_chunks = []

            for chunk in stream_message(session_id=session_id, message=question, context=context):
    ...
    def process_query(self, user_id: str, question: str, context_limit: int = 5) -> str:
        """
        Process a complete RAG query synchronously.
        ...
        # 4. Generate response
        response_chunks = []
        for chunk in stream_message(session_id=session, message=question, context=context):
...
async def rag_chat_stream_endpoint(user_id: str, question: str):
    ...
    def event_generator():
        for chunk in rag_service.generate_streaming_response(session, question, context):
            yield chunk
